...
export function useSync() {
  const pushChange = useCallback(async (tabla: string, tipo: 'CREATE' | 'UPDATE' | 'DELETE', payload: any) => {
    const change = {
      tabla,
      tipo,
      payload,
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (json && json.success) {
          await Promise.all(pending.map(p => db.changes.delete(p.id as number)));
        }
      } else {
  ...
  const flushPending = useCallback(async () => {
    ...
    for (const c of pending) {
      try {
        await fetch(`/api/${c.tabla}`, {
          method: c.tipo === 'CREATE' ? 'POST' : c.tipo === 'UPDATE' ? 'PUT' : 'DELETE',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(c.payload),
        });
        await db.changes.delete(c.id as number);
      } catch (e) {
        // keep it for later
      }
    }
